...
class WikiText2DataLoader:
    def __init__(self, config):
        ...
        self.vocab = build_vocab_from_iteration(map(tokenizer, train_iter), specials=['<unk>']) # indexing tokenizer to train_iter, <unk> means the word shown less than 2 times
        ...
        self.val_data = batchify(val_data, test_batch_size)
        self.test_data = batchify(test_data, test_batch_size)
    ...
    def data_process(self, raw_text_iter: dataset.IterableDataset) -> Tensor:
        ...
        """
        data=[torch.tesnor(self.vocab(self.tokenizer(item)), dtype=torch.log) for item in raw_text_iter]
